...
def render():
    ...
    with st.container(border=True):
        st.markdown("### 🌎 5. Demografia, Gênero e Desigualdades Regionais")
    ...
    if ano_f5 == "Plurianual (2021-2025)":
        with st.container(border=True):
            st.markdown("<strong>📊 Visão Plurianual Demográfica e Regional (2021–2025):</strong>", unsafe_allow_html=True)
            st.markdown("""
    ...
    # Containers lado a lado estruturados nativamente
    col_critica_f5, col_proposta_f5 = st.columns(2)
    with col_critica_f5:
        with st.container(border=True):
            st.markdown("#### 🔍 Análise Crítica Demográfica e Regional")
            st.markdown("""
