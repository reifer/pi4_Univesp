...
def process_enrichment_and_aggregations(spark: SparkSession, year: int = 2025, dict_path: str | None = None):
    print(f"\n=======================================================")
    ...
    if not os.path.exists(input_path):
        raise FileNotFoundError(f"Dataset limpo não encontrado em: {input_path}. Execute o ingest_all.py primeiro.")
    ...
    # Mapeamentos Sociodemográficos Descritivos
    ...
    # Carregamento do Dicionário JSON
    ...
    # Agregação 1: Rede de Ensino
    print(f"Gerando agregação da (NU_ANO x SG_UF_PROVA x TP_DEPENDENCIA_ADM_ESC_DESC)...")
    agg_rede = df_joined.groupBy("NU_ANO", uf_col, "TP_DEPENDENCIA_ADM_ESC_DESC").agg(
    ...
    agg_rede = agg_rede.withColumn("total_uf_ano", F.sum("total_candidatos").over(w_uf_ano)) \
                       .withColumn("percentual_alunos", F.round((F.col("total_candidatos") / F.col("total_uf_ano")) * 100.0, 2)) \
                       .drop("total_uf_ano")

    agg_rede.write.mode("overwrite").parquet(output_rede_path)

    # Agregação 2: Socioeconômico x Escola
    print("Gerando agregação avançada: Renda x Trabalho x Rede...")
    agg_socio_escola = df_joined.groupBy(
        "NU_ANO", "TP_DEPENDENCIA_ADM_ESC_DESC", "RENDA_FAMILIAR_DESC", "TRABALHO_COND_DESC"
    ).agg(
        F.count("*").alias("total_candidatos"),
        F.round(F.avg("NU_NOTA_CN"), 2).alias("media_cn"),
        F.round(F.avg("NU_NOTA_CH"), 2).alias("media_ch"),
    ...
    # Agregação 4: Notas por Renda Canônica
    ...
    # Agregação 5: Demografia Completa
